...
TITLE = "CS 21 17.2 - Project 2 Server v1.3"
MARS_PATH = "mars.jar"
...
class State(object):
    ...
    def _populate_registers(self):
        for regnum, val in self.regvalues.items():
            self.regfile[regnum] = val
    ...
    def _mars_dump(self, marspath, inputfile, outputfile, segment):
        command = "java -Dapple.awt.UIElement=true -jar {marspath} nc a sm {zero} dump {segment} HexText {outputfile} {inputfile}".format(
            marspath=marspath,
            inputfile=inputfile,
    ...
    def input_data_memory(self, addr, writedata, memread, memwrite):
        ...
        if self._bin_to_int(memwrite):
            self._write_mem_word(addr, writedata)

        # MemRead is ignored
        data = self._read_mem_word(addr)
    ...
    def _read_mem_word(self, addr):
        naddr = self._hex_to_int(addr)
        logger.debug("Reading word from memory address: %s (%s)" % (addr, naddr))

        ret = 0
        for i in range(4):
            ret <<= 8
            ret |= self._read_mem(naddr + i)
    ...
    def _write_mem_word(self, addr, data):
        naddr = self._hex_to_int(addr)
        ndata = self._hex_to_int(data)
        logger.debug("Writing word to memory address %s: %s (%s: %s)" % (addr, data, naddr, ndata))

        for i in range(4):
            self.mem[naddr + i] = ((ndata >> (8 * (4 - i - 1))) & 0xff)
...
def select_com_port():
    ...
    minimum_choice = min(len(com_ports), DEFAULT_PORT_CHOICE)

    # No need for validation
    choice = input("Select COM port [%s]: " % minimum_choice)
    if not choice:
...
def handle_start_message(state, ser):
    ...
    logger.info("Must show %s" % ((state.pc // 4) % 100))
...
def handle_input_message(ser, state, msg):
    ...
    logger.info("Stored PC is %s" % ((state.pc // 4) % 100))
...
def main():
    ...
    state = State(regvalues=REGISTER_VALUES)
